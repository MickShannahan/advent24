...
function partOne(updates, rules) {
  let out = 0
  const befores = {}
  rules.forEach(rule => {
    let key = rule[0]
    let val = rule[1]
    befores[key] = befores[key] ? [...befores[key], val] : [val]
  })
  const afters = {}
  rules.forEach(rule => {
    let key = rule[1]
    let val = rule[0]
    afters[key] = afters[key] ? [...afters[key], val] : [val]
  })
  ...
  updates.forEach(update => {
    ...
    for (let i = 0; i < update.length; i++) {
      let line = update.join(' ')
      let num = update[i]
      let [bef, aft] = line.split(num).map(t => t.trim())
      // console.log('--', bef, `[${num}]`, aft);
      // console.log('bs', aft.split(' '), befores[num]);
      // console.log('as', bef.split(' '), afters[num]);
      const isAfter = aft.split(' ').filter(n => n).every(n => befores[num]?.includes(parseInt(n)))
      const isBefore = bef.split(' ').filter(n => n).every(n => afters[num]?.includes(parseInt(n)))

      // console.log('==', `b${isBefore}`, `a${isAfter}`);
      if (isBefore && isAfter) {
        console.log('☑️', num);
      } else {
        correctLine = false
    ...
    if (correctLine) {
      console.log('line correct', update);
...
// partOne(testIn, testRule)
// partOne(Input, Rules) //6041



function partTwo(updates, rules) {
  let out = 0
  const befores = {}
  rules.forEach(rule => {
    let key = rule[0]
  ...
  updates.forEach(update => {
    ...
    if (!correctLine) {
      // console.log(middle);
      // console.log('INcorrcet', update);
      const clone = update.map(n => parseInt(n))
      clone.sort((a, b) => {
        // console.log('A', a, befores[a], afters[a]);
        // console.log('B', b, befores[b], afters[b]);
        if (befores[a]?.includes(b)) {
          console.log('moving up', a);
          return -1
        } else if (afters[a]?.includes(b)) {
          return 1
        }
        return 0
      })
      let middle = clone[~~(clone.length / 2)]
      console.log('🛠️', clone, middle);
      out += middle
    }
  })
  console.log('🎄', out);
...
partTwo(testIn, testRule)
partTwo(Input, Rules) //4884